...
- **Invariante de integridad del plan** (`planIsValid(plan)` = `plan.weeks[]` no vacío): defense-in-depth contra un `gym_plan.data = {}` corrupto que hidrataba `DB.plan = {}` → `nextSession()` truena / muestra "Plan completado" falso → Plan/Hoy en blanco. Un plan inválido **nunca** se persiste (`gymStateRows` → `null`), ni se hidrata (`hydrateGymDB` → `null`), ni pisa uno bueno vía realtime (`applyGymRealtime` gym_plan ignora `{}`), ni truena en render (`nextSession`/`renderPlan` lo tratan como "sin plan"). **Auto-sana**: si hay `profile.baseline`, `bootWithSession` + `nextSession` regeneran el plan desde baseline → el próximo push sobrescribe el `gym_plan` vacío en la nube. Bug real: el `gym_plan` remoto de Andrés quedó en `{}` (origen no determinable por lectura estática; posible push transitorio / versión vieja / eco realtime) — la invariante lo repara en el próximo boot sin importar el origen.
...
- **Realtime**: `subscribeGymRealtime()` — canal `gym-sync`, `postgres_changes` por tabla filtrado por `user_id`. Cambio entrante → `applyGymRealtime` → si cambió, persiste + **re-render de la pantalla actual** (`.nav-btn.active`, no cambia de tab). Last-write-wins por fila.
